...
def solve(input_data):
    ...
    '''
    provinces_startswith_h = [(d['name'],d['population']) for d in input_data if d['name'].startswith('H')]
    provinces_more_than_1m = [(d['name'],d['population']) for d in input_data if d['population'] > 1000000]
    provinces_startswith_h = sorted(provinces_startswith_h, key = lambda x : x[0], reverse = True)
    provinces_more_than_1m = sorted(provinces_more_than_1m,key = lambda y : y[1],reverse = True)
